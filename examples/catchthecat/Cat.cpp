...
Point2D Cat::Move(World* world)
{
	...
	while (!queue.empty())
	{
		...
		for (Point2D n : currentNeighbours)
		{
			...
			{
				// backtrack to find the appropriate next step
				Point2D currentPoint = head.position;
				while (from[currentPoint.x][currentPoint.y] != world->getCat())
				{
					currentPoint = from[currentPoint.x][currentPoint.y];
				}
				return currentPoint;
			}
